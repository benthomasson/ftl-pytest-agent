...
from smolagents.utils import _is_pillow_available
...
args_split_re2 = re.compile(
    r"""
(?:^|\n)  # Match the start of the args block, or a newline
\s*(\w+)\s*\(\w+\):\s*  # Capture the argument name and strip spacing
(.*?)\s*  # Capture the argument description, which can span multiple lines, and strip trailing spacing
(?=\n\s*\w+\s*\(\w+\):|\Z)  # Stop when you hit the next argument or the end of the block
""",
    re.DOTALL | re.VERBOSE,
)
